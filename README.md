...
It is well known that the inverse of the measured parallax is not an unbiased estimator for the distance to a star, due in part to the non-linearity of the transformation and the asymmetry of the resulting probability distribution ([Lutz & Kelker 1973](https://ui.adsabs.harvard.edu/abs/1973PASP...85..573L); [Smith 2003](https://ui.adsabs.harvard.edu/abs/2003MNRAS.338..891S) and [Luri et al. 2018](https://ui.adsabs.harvard.edu/abs/2018A%2526A...616A...9L)). To mitigate these biases we can make use of Bayesian inference, combining the observed parallax with a prior distance distribution, allowing us to update our "degrees of belief" about the plausible distances. Currently, the most cited work doing this for Gaia DR3 data is [Bailer-Jones et al. (2021)](https://ui.adsabs.harvard.edu/abs/2021AJ....161..147B), which models its prior based on the GeDR3 mock catalogue from [Rybizki et al. (2020)](https://ui.adsabs.harvard.edu/abs/2020PASP..132g4501R) (which consists of simulated data mimicking the expected contents of Gaia EDR3 prior to its actual publication).

However, there are two primary reasons why the "Bailer-Jones distances" are not the best that can be obtained from Gaia DR3 when dealing with the OB stellar population:
- Although the parallax zero-point corrections from Lindegren et al. (2021a) were applied in Bailer-Jones et al. (2021), they did not incorporate the more recent improvements for bright stars outlined by Maíz Apellániz (2022), which are relevant for bright stars. Furthermore, the well-documented underestimation of the total parallax uncertainty (including both random and systematic components) in Gaia (E)DR3 (Maíz Apellániz et al. 2021) was not considered in that work either, as this analysis was unavailable at the time of publication.
- The chosen prior distance distribution in Bailer-Jones et al. (2021) is highly detailed, having both a longitudinal and latitudinal dependence. But the prior is informed by the general population observed by Gaia, which is overwhelmingly over-represented by late-type dwarfs (in the near field) and red giants (at larger distances). Since our focus is on young, massive stars, the prior must reflect the distinct spatial distribution of OB stars, including the shorter scale height of the Galactic thin disc. As the authors of that paper note: "If one focuses on a restricted set of stars with some approximately known properties, it will be possible to construct more specific priors and to use these to infer more precise and more accurate distances". This is exactly what is done by our code.

To address both issues one can build a prior based on the spatial distribution of OB stars derived from Hipparcos data ([van Leeuwen 2007](https://ui.adsabs.harvard.edu/abs/2007ASSL..350.....V)), modeled as an infinite, thin, isothermal, self-gravitating disc with a scale height of $h_d= 31.8$ pc, along with an additional Gaussian halo component (primarily consisting of massive runaway stars) with a scale height of $h_h = 490$ pc, as viewed from the Sun, which is assumed to be located at $z = 20$ pc above the mid-Galactic plane. This prior model was built from [Maíz Apellániz (2001)](https://ui.adsabs.harvard.edu/abs/2001AJ....121.2737M), [Maíz Apellániz (2005)](https://ui.adsabs.harvard.edu/abs/2005ESASP.576..179M) and [Maíz Apellániz et al. (2008)](https://ui.adsabs.harvard.edu/abs/2008arXiv0804.2553M).
